...
def create_user(user: UserCreate, db: Session = Depends(get_db)):
    db_user = dao.get_user_by_username(db, username=user.username)
    if db_user:
...
def get_my_posts(skip: int = 0,
    ...
    return dao.get_users_posts(db, author_id=user.id, skip=skip, limit=limit)
...
def get_users_posts(user_id, skip: int = 0, limit: int = 100, db: Session = Depends(get_db)):
    posts = dao.get_users_posts(db, author_id=user_id, skip=skip, limit=limit)
    return posts
